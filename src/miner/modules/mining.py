...
class BBSCrawler(object):
    '''
    @author: Paul Yang
    @note: This prog is to fetch the ptt's content based on the board name
            like car  and the fetched files will be stored under the
            directory "./fetched/BOARDNAME/"
    @since: 2014/8/2, v0.2
    '''

    def __init__(self, board_name='car', myPageNum=10, fetch_path='./', toNum=0,
                 debugFlag=False, forAll=False):
        '''
        Constructor
        '''
        ...
        # put the cookie header for the board like Gossiping to pass around
        # the limit of 18 age
        ...
        # if forAll is on, iterate the total number of pages for the board
        # by getAllPagesInTheBoard()
        self.forAll = forAll
        self.toNum = toNum
        self.fetch_path = fetch_path
        self.path = os.path.join(self.fetch_path, self.board_name)
        self.ESPECIAL_URL =
                'http://www.ptt.cc/bbs/%s/index.html' % self.board_name
        self.post_url =
                lambda id: 'http://www.ptt.cc/bbs/%s/%s.html' % (
                        self.board_name, id)
        self.page_url =
                lambda n: 'http://www.ptt.cc/bbs/%s/index%s.html' % (
                        self.board_name, str(n))
        self.initLogging()
    ...
    def getAllPagesInTheBoard(self):
        ...
        try:
            if (self.useHeader):
                ...
                indexPage = bs4.BeautifulSoup(
                        urllib2.urlopen(self.ESPECIAL_URL).read(), "lxml")
            href = indexPage.find_all("a", class_="btn wide")[1].get('href')
            self.allPageNums = int(re.sub(r'[^0-9]+', '', href))
    ...
    def getContent(self):
        ...
        if self.toNum != 0 and not self.forAll:
        ...
        # add the index to record the total number that has processed, the
        # number of failure and the number of success.
        ...
        with open('Metadata_json','w+') as metaDataFp:
            # iterate through index page like "www.ptt.cc/bbs/car/index.html"
            # to get each POST ID for indexP in xrange(1, pagesToRun + 1):
            for indexP in range(startIndex, endIndex):
                ...
                try:
                    if self.useHeader:  # if the page require header
                        request = urllib2.Request(self.page_url(indexP),
                                                  headers=self.headers)
                        page = bs4.BeautifulSoup(
                                urllib2.urlopen(request).read(), "lxml")
                    else:
                        page = bs4.BeautifulSoup(
                                urllib2.urlopen(self.page_url(indexP)).read(),
                                "lxml")
                except:
                ...
                for link in page.find_all(class_='r-ent'):
                    ...
                    # Fetch the post content via post id,
                    # ex. http://www.ptt.cc/bbs/car/M.1400136465.A.DD5.html
                    ...
                    try:
                        ...
                        if (self.useHeader):  # if the page require header
                            request = urllib2.Request(self.post_url(post_id),
                                                      headers=self.headers)
                            post = bs4.BeautifulSoup(
                                    urllib2.urlopen(request).read(), "lxml")
                        else:
                            post = bs4.BeautifulSoup(urllib2.urlopen(
                                    self.post_url(post_id)).read(), "lxml")
                    except:
                        sys.stderr.write('Error occured while fetching %s\n' %
                    ...
                    with open(post_id, 'w') as contentFile_fp:
                        try:
                            strr = self.remove_html_tags(
                                str(post.find(id='main-container')))
                            s = '發信站: 批踢踢實業坊(ptt.cc)'
